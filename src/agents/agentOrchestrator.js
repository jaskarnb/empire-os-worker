...
function voiceDirection(style, audience) {
  if (style === "horror") return "mix formats: low suspense narration for 35-59 second scary stories, and minimal voice with sharp sound design for 12-20 second jump-scare clips";
...
function formatMix(style) {
  if (style === "horror") {
    return [
      {
        id: "short-jump-scare",
        targetLength: "12-20 seconds",
        cadence: "most posts",
        direction: "One scary caught-on-camera moment with fast setup, quiet tension, a strong platform-safe jump scare, scary sound, and a short aftermath beat. Never make it feel like an 8-second preview.",
      },
      {
        id: "long-scary-story",
  ...
  if (style === "kids") {
    return [{ id: "cheerful-story", targetLength: "12-35 seconds", cadence: "default", direction: "Bright motion, cheerful voice, simple captions, safe joke or payoff, no scary beats." }];
  }
  if (style === "brainrot") {
    return [{ id: "fast-chaos", targetLength: "12-30 seconds", cadence: "default", direction: "Fast motion, loud retention beats, meme pacing, exaggerated captions, and no dead air." }];
  }
  return [{ id: "complete-short", targetLength: "12-45 seconds", cadence: "default", direction: "Motion-first short with hook, escalation, payoff, captions, and platform-native pacing." }];
}
...
function researchRule(style) {
  ...
  if (style === "horror") return `${base} For horror, compare 12-20 second jump-scare clips against 35-59 second scary-story videos and choose a mix based on what is working now.`;
...
function postingTimeRule(style) {
  ...
  if (style === "horror") return `${base} Horror should favor evening and late-night windows first, especially 7:30 PM-11:30 PM, with 12-20 second jump-scare clips and 35-59 second story videos mixed across separate slots.`;
...
Target runtime: 12-59 seconds.
Rotate between two winning formats:
1. Short jump-scare clip, 12-20 seconds: fast setup, silence, sudden scary reveal, frightening sound hit, and a short aftermath beat.
...
Target runtime: 12-59 seconds. Never make it shorter than 12 seconds.
...
function assignments({ niche, style, audience, referenceAnalysis, spend }) {
  ...
  const horrorFormatNote = style === "horror" ? " Rotate formats between 12-20 second jump scares and 35-59 second narrated scary stories with captions and scary sound." : " Follow the same quality bar as horror: 12-59 seconds, motion-first, entertaining, niche-matched sound/voice, not static caption posts.";
  ...
  return [
    ...
    { agent: "quality-gate", task: "Reject weak/static/unsafe videos before posting. Require real motion, readable captions when used, niche-matched sound, 12-59 second runtime, and no static image posts. Also reject anything too close to a creator reference.", output: "pass/fail plus regeneration notes", status: "required" },
